    ...
    {   //  Si le POST n'est pas vide, ça veut dire que c'est pas mon 1er passage mais que c'est mon 2eme passage.
        ...
        else
        {
            ...
            {       // Alors tu vas commencer à travailler. | !move_upload_file => c'est une fonction qui va : $_FILES["image"]["tmp_name"] => prendre mon fichier 
                if(!move_upload_file($_FILES["image"]["tmp_name"], $imagePath)) // Pour le moment, notre fichier se trouve dans une chose temporaire donc ici, on va le définir avec le tmp_name (temps_name) Donc je vais dire : tu vas prendre cette image temporaire et tu vas me la mettre dans le vrai chemin que j'ai défini.
